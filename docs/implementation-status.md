...
Item 1's other two items (Zoho Sign send/resync-agreement zero auth — §2.19; webhook signature fails open — §2.20), item 1's `profiles` export/read leaking `password_hash` via the server ignoring column allowlists (§2.17), and every item from Phase 2 onward: the per-table `getScopeSpec` review (§2.1/§2.2/§2.4/§2.9/§2.10/§2.22), the deal pipeline core-invariant fixes (§2.5-§2.8), ticketing outage + reward integrity (§2.13-§2.16), notifications/News (§2.11-§2.12), config guardrails (§2.18), and the large rebuilds (Integration Operations Centre, Auto CRM, ticketing completeness, Insight Hub assessments/certificates, Zoho Books/WhatsApp/GyFTR/DHL adapters). Also idle-timeout session enforcement (noted above), rate limiting (§19d — not bundled into this batch to keep it scoped), MFA, and everything else in `current gaps.md`'s S2/S3 tables.
