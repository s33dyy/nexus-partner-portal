...
export async function addDealLineItem(input: {
  ...
  return withTransaction(async (tx) => {
    ...
    if (deal.stage !== "sourced" && deal.stage !== "demo" && deal.stage !== "testing" && deal.stage !== "qualified" && deal.stage !== "proposal") {
      return { ok: false, failure: validationFailure("Cannot modify line items at this stage"), correlationId };
    }
...
export async function removeDealLineItem(input: {
  ...
  return withTransaction(async (tx) => {
    ...
    await tx.query(
      ...
      [input.data.lineItemId, input.data.dealId]
    );
