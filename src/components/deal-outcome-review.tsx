...
import { submitPO, approvePO, requestChanges, rejectOutcome } from "@/integrations/local/outcome-review-commands";
import { toast } from "sonner";
import { FileUp, Check, X, AlertCircle, Loader2 } from "lucide-react";
import { useAuth } from "@/hooks/use-auth";

export function DealOutcomeReview({ dealId, dealStage, commercialApproved }: { dealId: string, dealStage: string, commercialApproved: boolean }) {
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Deal must be commercially approved and in negotiation before submitting a PO.</CardDescription>
        </CardHeader>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Submit and review purchase orders to mark this deal as Won.</CardDescription>
      </CardHeader>
        ...
        {isNegotiation && (
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input value={poUrl} onChange={e => setPoUrl(e.target.value)} placeholder="https://..." />
            </div>
        ...
        {isSuperAdmin && (
          <div className="p-4 bg-muted rounded-md space-y-4 border">
            ...
            <div className="flex gap-2">
              <Button size="sm" variant="default" onClick={() => handleAction("approve")} disabled={loading}>
                <Check className="w-4 h-4 mr-2" /> Approve & Win Deal
              </Button>
              <Button size="sm" variant="secondary" onClick={() => handleAction("changes")} disabled={loading}>
                Request Changes
              </Button>
