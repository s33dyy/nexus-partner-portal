...
async function seedDocumentBlob(
  ...
  const upload = await uploadDocumentBlob({
    ...
    // @types/node's buffer.File shadows the DOM File this expects; Bun's
    // runtime File is the Web one, so the shapes match where it matters.
    file: new File([new Uint8Array(fileBuffer)], fileName.toLowerCase().replace(/\s+/g, "-"), {
      type: "application/pdf",
    }) as unknown as Parameters<typeof uploadDocumentBlob>[0]["file"],
    isSeed: true,
  });
  return {
