...
test("parseIntent falls back safely for unparseable model output without echoing raw text", () => {
  const intent = parseIntent("I'm not sure what you mean by that.");
  expect(intent.type).toBe("none");
  if (intent.type === "none") {
    expect(intent.reply).toContain("couldn't process");
    expect(intent.reply).not.toContain("not sure what you mean");
  }
});
