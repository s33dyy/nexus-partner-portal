...
import { frontendUrl } from "@/lib/app-urls";
import { getRequestToken } from "@/middleware/auth-context";
...
import type {
  ...
  LocalUser,
  PartnerStatus,
} from "@livey/shared/types/auth";
import type { TableQuery as ClientTableQuery } from "@livey/shared/types/table-query";

export type { AppRole, PartnerStatus, LocalUser, LocalSession };
...
/** The client-supplied half (see @livey/shared/types/table-query) plus the
 * read-scope narrowing only applyTablePolicy may set. */
export type TableQuery = ClientTableQuery & {
...
export async function getSessionFromToken(token: string): Promise<IssuedSession | null> {
...
export async function requestPasswordReset(email: string) {
  ...
  if (profile) {
    ...
    console.info(
      `[password-reset] requested for ${profile.email}: ${frontendUrl()}/reset-password?token=${token}`,
    );
  }
...
export async function issueTemporaryPasswordForUser(userId: string) {
  ...
  const { generateTemporaryPassword } = await import("@livey/shared/lib/temp-password");
...
export async function createDocumentDataUrl(filePath: string) {
  ...
  const blob = result.rows[0] as
    { file_name: string; mime_type: string; file_data: Buffer } | undefined;
  if (!blob) {
