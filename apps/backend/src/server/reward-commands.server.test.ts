...
// Captured before any test calls mock.module("@/lib/gyftr", ...) so
...
const realGyftrModule = await import("@/lib/gyftr");
...
test("approveRewardRedemption records the provider's own voucher reference when GyFTR issuance succeeds", async () => {
  ...
  try {
    ...
    mock.module("@/lib/gyftr", () => realGyftrModule);
    harness.restore();
  }
});
