...
test("Google callback logs in an existing user matched by google_id", async () => {
  ...
  try {
    ...
    expect(location).toBe(`${FRONTEND}/auth/callback`);
...
test("Google callback auto-links an existing password account on its first Google login (verified email)", async () => {
  ...
  try {
    ...
    expect(response.status).toBe(302);
    expect(response.headers.get("Location") ?? "").toBe(`${FRONTEND}/auth/callback`);
  ...
  try {
    ...
    const response = await handleGoogleCallback(
      callbackRequest(
        ...
        `${STATE_COOKIE_HEADER}; google_oauth_actor=${currentUser.id}`,
      ),
    );
    expect(response.status).toBe(302);
    expect(response.headers.get("Location")).toBe(`${FRONTEND}/settings?googleConnected=1`);
