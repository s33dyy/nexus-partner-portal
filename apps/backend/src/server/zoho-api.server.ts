...
async function requireSuperAdminSession(request: Request): Promise<Response | null> {
  const { profile, roles } = await getAuthContext(readSessionToken(request) ?? undefined);
...
export async function handleZohoCallback(request: Request) {
  ...
  try {
    ...
    const redirectUrl = new URL(frontendRedirect("/settings"));
  ...
  const successUrl = new URL(frontendRedirect("/settings"));
...
export async function handleZohoSignUrl(request: Request) {
  try {
    const { profile } = await getAuthContext(readSessionToken(request) ?? undefined);
    if (!profile?.id) {
      return new Response(JSON.stringify({ error: "Unauthorized" }), {
        status: 401,
        headers: { "Content-Type": "application/json" },
