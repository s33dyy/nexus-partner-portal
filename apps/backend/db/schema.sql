...
-- Single-use handoff for Settings' "Connect Google" flow. The SPA trades an
-- authenticated API call for a ticket so the OAuth callback has an explicit
-- initiating actor instead of relying on an ambient browser session.
CREATE TABLE IF NOT EXISTS google_oauth_connect_tickets (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES profiles(id) ON DELETE CASCADE,
